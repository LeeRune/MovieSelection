...
class MovieHomeVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        auth = Auth.auth()
        getMoiveInfo()
        memberButton.setPreferredSymbolConfiguration(UIImage.SymbolConfiguration(pointSize: 50), forImageIn: .normal)
    }
    ...
    @IBAction func memberButton(_ sender: Any) {
        if auth.currentUser != nil {
            let membervc = storyboard?.instantiateViewController(withIdentifier:
                    "MemberVC") as! MemberVC
